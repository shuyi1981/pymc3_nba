...
if os.path.isfile(output + feed + "\\" + 'log.csv'):
    already_downloaded = pd.read_csv(output + feed + "\\" + 'log.csv', names = ['date', 'status'], index_col=None)
else:
...
for date in dates_to_download:
    ...
    #     fp.write(str(game))
    completed = already_downloaded[already_downloaded['status'] == " downloaded"]
    if date not in completed.date.unique():
        trial = 1
        while trial < max_tries:        
            try:
                data = get_data(version=version,league=league, season=season, feed=feed, format=format, api=msf, date=date)
                status = "downloaded"
                trial = max_tries
            except:
                print(date)
